...
export const id = 'fileboom';
export const label = 'FileBoom';
...
const API_BASE_URL = 'https://fboom.me/api/v2';
...
async function postJson(endpoint, body) {
  ...
  try {
    ...
    throw new Error(`FileBoom ${endpoint} returned non-JSON response: ${text.slice(0, 500)}`);
  }
  if (!response.ok || data.status === 'error') {
    throw new Error(`FileBoom ${endpoint} failed: ${JSON.stringify(redactAuth(data))}`);
  }
  return data;
}

export async function login({ username, password, recaptchaResponse }) {
  if (!username || !password) throw new Error('FileBoom username/password missing.');
  ...
  if (!authToken) {
    throw new Error(`FileBoom login did not return token: ${JSON.stringify(redactAuth(data))}`);
  }
...
async function getUploadFormData({ accessToken, authToken, parentId = DEFAULT_PARENT_ID }) {
  ...
  if (!data.form_action || !data.file_field || !data.form_data) {
    throw new Error(
      `Unexpected FileBoom upload form response: ${JSON.stringify(redactAuth(data))}`,
    );
  }
  return data;
}
...
export async function upload({ filePath, credentials = {}, onProgress }) {
  absolutePathRequired(filePath, 'fileboom.upload()');
  ...
  if (!accessToken && !authToken) {
    throw new Error('FileBoom requires accessToken, authToken, or username/password.');
  }
  ...
  try {
    ...
    throw new Error(`FileBoom upload returned non-JSON response: ${text.slice(0, 500)}`);
  }
  if (!response.ok || data.status === 'error' || data.success === false) {
    throw new Error(`FileBoom upload failed: ${JSON.stringify(redactAuth(data))}`);
  }

  const plainText = String(data.link || '').replace(/^http:\/\//i, 'https://');
  if (!plainText) {
    throw new Error(`FileBoom upload did not return link: ${JSON.stringify(redactAuth(data))}`);
  }
